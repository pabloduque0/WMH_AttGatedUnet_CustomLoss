...
//        fprintf(fp, "%f ", data[n*(rows*i+j)+k]);
          fprintf(fp, "%f ", data[n*(cols*i+j)+k]);
      }
    }
    for(k = 0; k < n-1; k++) {
//      fprintf(fp, "%f ", data[n*(rows*i+cols-1)+k]);
        fprintf(fp, "%f ", data[n*(cols*i+cols-1)+k]);
    }
//    fprintf(fp, "%f\n", data[n*(rows*i+cols-1)+n-1]);
    fprintf(fp, "%f\n", data[n*(cols*i+cols-1)+n-1]);
    }
  fclose(fp);  
  return NO_ERRORS;
}
...
{  
  ...
  default:
    if(filetype == FILE_UNKNOWN || filetype == EXE_FILE_READ_ERROR) return EXE_UNKNOWN_FILE_FORMAT;
  }
